from msight_vision.msight_core import RFDETRDetectionNode
from msight_core.utils import get_node_config_from_args, get_default_arg_parser
import time

def main():
    parser = get_default_arg_parser(description="Launch RF-DETR Detection Node", node_class=RFDETRDetectionNode)
    ...
    detection_node = RFDETRDetectionNode(
        configs,
        args.det_configs,
    )
    detection_node.spin()
